// app.js v0-1 — 許可直後にゲートを必ず閉じる/失敗時のみ再表示。E線含む検出レンジ維持。
...
async function openMic(){
  ...
  // A-weighting近似（低域カット＋2.5k周辺ピーク）— E線(1.3–1.6kHz)も確実に拾う
...
// —— UI / ライフサイクル —— //
// 「許可」クリック直後にまず閉じて、失敗時のみ再表示（Safariでの”閉じない”対策）
...
// 裏に回ったらマイクOFF、復帰時は「未接続のときだけ」ゲートを出す
