// app.js v0-4c-gh — 既存機能維持。針式チューナー改良、48音固定、0.5秒保持、許可ボタン多重ガード。
...
(function buildTicks(){
  ...
  const ang = c=>(-50+c)/100* (Math.PI*0.9) - Math.PI*0.45;
  for(let c=-50;c<=50;c+=5){
    const a=ang(c); const inner=r-(c%25===0?18:10);
...
// —— マイク許可：押せない端末対策（pointerup/touchend/click＋グローバル関数） —— //
window.__permit = async function(){
  const ok = await openMic();
  if (ok) resetSession(selKey.value);
