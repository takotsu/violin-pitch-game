...
const PASS_DWELL_MS = 100;   // 合格滞留時間（従来の半分）
const COOL_NEXT_MS = 100;    // 次音へ進むクール（従来200→100）
const LOCK_SAME_MS = 140;    // 同音ロックをやや短縮
...
function drawPage(){
  ...
  // キャンバスを譜面サイズに合わせる（高音域見切れ対策）
  sparkCanvas.width = staffWrap.clientWidth;
  sparkCanvas.height = staffWrap.clientHeight;
}
...
function autoCorrelateF0(frame, sr){
  ...
  for(let tau=tauMin; tau<=tauMax; tau++){
    let sum=0; for(let i=0;i<N-tau;i++){ const diff = frame[i]-frame[i+tau]; sum += diff*diff; } d[tau]=sum;
  }
...
function servoLoop(ts){
  ...
  lastServoTS = ts;
  servoUpdate(nearestSemiCents, dt);
...
function startLoops(){
  ...
  (function loop(){
    ...
    if(!cur){ rafPitch = requestAnimationFrame(loop); return; }
    ...
    if(absC<=PASS_BAND_CENTS){
      ...
      const dwell = now - inBandSince;
      if(sc>=passThreshold && dwell>=PASS_DWELL_MS && now - lastAdvance >= Math.max(COOL_NEXT_MS, LOCK_SAME_MS)){
...
function badgeFor(sc){
  ...
  burstAtNote(iInPage); // 視覚的フィードバック強化
}
function advance(){
  idx++;
...
/* ===== ゲート・ライフサイクル（バックグラウンド即停止＋取りこぼし監視） ===== */
