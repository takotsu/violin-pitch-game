...
let audio, analyser, srcNode, gain, shelf, band, mediaStream;
let rafId=null, running=false, session=null, renderCtx=null;

async function openMic(){
  // Appleの仕様：ユーザー操作（permit/start）から呼ぶ
  ...
  shelf = audio.createBiquadFilter(); shelf.type="highshelf"; shelf.frequency.value=1200; shelf.gain.value=9;
  band  = audio.createBiquadFilter(); band.type="bandpass"; band.frequency.value=700; band.Q.value=0.7;
  gain  = audio.createGain(); gain.gain.value=1.0;
  analyser = audio.createAnalyser(); analyser.fftSize=4096; analyser.smoothingTimeConstant=0.04;
...
function closeMic(){
  ...
  analyser=srcNode=gain=shelf=band=undefined;
}

const buf=new Float32Array(8192);
function detectPitch(){
  if(!analyser) return {freq:0,rms:0,vu:0};
  ...
  // dBFS→VU(0–100) へ変換（-90..0 dBFSを 0..100 に線形マッピング）
  const dbfs = rms>0 ? 20*Math.log10(rms) : -120;
  const vu   = Math.max(0, Math.min(100, Math.round((dbfs+90)/90*100)));
  micvu.textContent = `入力 ${vu}/100`;
  micvu.title = `RMS=${rms.toFixed(4)}  dBFS=${dbfs.toFixed(1)}`;

  if(rms < parseFloat(inpRMS.value||"0.002")) return {freq:0,rms,vu};
  ...
  if(bestLag>0){
    ...
    return {freq: sr/lag, rms, vu};
  }
  return {freq:0,rms,vu};
}
...
permit.addEventListener("click", async ()=>{
  try{ await openMic(); gate.setAttribute("aria-hidden","true"); }
  catch{ /* 失敗時はゲートを閉じない */ }
});

btnStart.onclick=async ()=>{
  try{
    if(!mediaStream) await openMic(); // 明示的に毎回要求
...
window.addEventListener("visibilitychange",()=>{ // バックに行ったら完全停止（マイクも切る）
