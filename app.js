...
/* ====== 設定 ====== */
const A4_REF_HZ = 442;
const STUCK_SEC_BEFORE_AUTONEXT = 3; // 停滞3秒＋次の強拍で自動的に1音送る
...
async function ensureVexFlow(){
  ...
  pushError(new Error("VexFlowをCDNから読み込めませんでした（フォールバック描画使用）。"));
  return false;
}
...
function adviceFromCents(c){
  const a=Math.abs(c);
  if(a<4) return "そのままキープ。";
  if(c>0){ // 高い
    if(a<10) return "ほんの少し低く。";
    if(a<20) return "少し低く。";
    if(a<35) return "もう少し低く。";
    return "かなり高いです。指をわずかにナット側へ。";
  }else{    // 低い
    if(a<10) return "ほんの少し高く。";
    if(a<20) return "少し高く。";
    if(a<35) return "もう少し高く。";
    return "かなり低いです。指をわずかに駒側へ。";
  }
}
...
function startMetronome(){
  ...
  metroTimer=setInterval(()=>{
    ...
    // 停滞ウォッチドッグ：一定時間進まず、直近で有効検出があれば次の強拍で自動送り
    if((beat%4===0) && sessionRunning){
      ...
      if(now - lastProgressTime > STUCK_SEC_BEFORE_AUTONEXT*1000 && now - lastValidTime < 1200){
        ...
        else { highlightIndex(renderCtx, idx); progEl.textContent=`音 ${idx+1}/${scaleData.noteObjs.length}`; lastProgressTime=now; }
      }
    }
    beat++;
  }, beatMs);
...
function onWorkletMessage(ev){
  ...
  if(rms < LEVEL_RMS_THRESHOLD){ feedbackEl.textContent='入力が小さいです。マイクを近づけてください。'; return; }
  if(!f0 || conf<0.5){ feedbackEl.textContent='検出が不安定です。一定の弓圧で。'; return; }
  ...
  // 助言表示のみ（点数は出さない）
  feedbackEl.textContent = adviceFromCents(cents);
  colorMini(Math.abs(cents));
...
function startSession(){
  ...
  try{
    ...
    idx=0; lastProgressTime=performance.now(); lastValidTime=0;
...
function finishScale(){
  ...
  const msgs=["いい流れでした！","集中力が素晴らしいです！","音程の安定感が増しています！","次はビブラートを合わせましょう。"];
  praiseEl.textContent = msgs[Math.floor(Math.random()*msgs.length)];
  detailsEl.textContent = `お疲れさまでした。`;
