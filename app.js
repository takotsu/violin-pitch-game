...
const COOLDOWN_NEXT_MS = 220;      // 合格後この時間は次に進ませない（連続同音対策）
const PASS_BAND_CENTS = 15;        // 合格帯（±cents）。高すぎるとF#取りこぼしが出やすい
...
async function ensureVexFlow(){
  ...
  pushError(new Error("VexFlowをCDNから読み込めませんでした（フォールバック描画を使用）。"));
  return false;
}
...
let mustExitPassBand=false; // 合格帯から一度出るまで次の通過を許さない
let scores=[]; // 初回通過スコアを記録
...
function setAdvice(cents, score, perfect=false){
  const abs=Math.abs(cents);
  const arrow = cents>0 ? "↑" : (cents<0 ? "↓" : "＝");
  const text = `${(abs|0)}c ${arrow}`;
  adviceEl.className="";
  if(perfect){ adviceEl.classList.add('good'); adviceEl.textContent="100点！完璧。響きが美しい。"; return; }
...
function onWorkletMessage(ev){
  ...
  if(!f0 || conf<0.5){ adviceEl.className="warn"; adviceEl.textContent='検出が不安定…'; return; }
  ...
  if(sc >= passThreshold && inPassBand && !mustExitPassBand){
    ...
    mustExitPassBand = true; // 次は一旦外れるまで不可
