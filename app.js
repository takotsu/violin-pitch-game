// app.js (変更あり)
// 🎮モード：C6以下・連続で同じ音名（#/b含む）は禁止（オクターブ違いも禁止）・30問。
// マイクは「開始→結果表示」間のみON。停止/離脱/非可視で完全停止。停止後に許可ゲートは自動表示しない。
...
// 🎮アーケード（30問、C6以下・G3以上、直前と同じ音名(#/b含)は不可）
...
function loadExercise(){
  ...
  ui.modeName.textContent = state.mode==="arcade" ? "🎮 30問" : "音階";
  resetClock();
}
function resetClock(){
...
// ボタン
ui.start.addEventListener("click", ()=> {/* ゲート表示は index.html 内 */});
...
ui.game.addEventListener("click", ()=>{
  ...
  if(pressed){
    ...
    state.mode = "arcade";
    loadExercise();
    showToast("🎮 ランダム30問（C6まで・連続同音名禁止）", "info", true);
  }
});
...
function fireworkFor(score, centsAbs, xy){
  ...
  addBurst(xy.x, xy.y, {count, life: 2400, color: col, big: (score>=98?2.0:1.5)});
  hudFlash(flash, score>=99?1: score>=95?0.85:0.65);
  if(centsAbs<=0.5){ addOcto(xy.x, xy.y-10, true); }
}
...
function goNextNote(){
  ...
  if(state.idx >= state.total){
    ...
    const modeStr = state.mode==="arcade" ? "モード: 🎮 ランダム30問" : "モード: 音階";
    const diffText = $("#difficulty").selectedOptions[0]?.textContent || "";
    ui.resultTitle.textContent = state.mode==="arcade" ? "🎮 ランダム30問 完了" : "音階 完了";
