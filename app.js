...
const PASS_DWELL_MS = 100;
const COOL_NEXT_MS = 100;
const LOCK_SAME_MS = 140;
...
function stopAudioSync(){ // 遷移時の同期停止（iOSのBFCache対策）
  try{
    stopLoops();
...
function startLoops(){
  ...
  (function loop(){
    ...
    nearestSemiCents = Math.max(-50, Math.min(50, centsDiff(f0,fSemi)));
...
/* ★要求：調を選択した時点で即描画（セッション中でも即反映） */
keySel.addEventListener('change', ()=>{
  currentKey = keySel.value;
  exercise = makeExercise4Bars(currentKey);
  pageOffset=0; idx=0; scores=[];
  drawPage(); updateProgress();
  toast(`キーを ${currentKey} に切替えました。`,'info',1200,true);
});
...
window.addEventListener("pagehide", hardStopSync, {passive:true});   // BFCacheでも確実停止
...
(function init(){
  ...
  try{
    ...
    if(problems.length){ problems.forEach(k=>console.warn("音域外検出(要バグ報告):",k)); }
  }catch(e){ logError(e); }
})();
