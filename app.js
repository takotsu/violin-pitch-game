...
function hudFlash(color="rgba(34,197,94,.45)", intensity=1){
  ...
  setTimeout(()=>el.classList.remove("show"), 500); // 0.5秒固定
}
...
function makeArcadeSet(){
  ...
  for(let i=0;i<20;i++){
    ...
    while(sig===prev){ p=pick(); sig=`${p.letter}${p.acc}${p.octave}`; } // 連続禁止
    out.push(p); prev=sig;
  }
  return out;
}
...
function addBurst(x,y,{count=380, life=1900, color="hsl(140,100%,65%)", big=1.4}={}){
  const cvs = ui.spark;
  const spread = 1 + big*0.8;
  for(let i=0;i<count;i++){
    const ang = Math.random()*Math.PI*2;
    const speed = spread*(1.2 + Math.random()*5.2);
    sparks.push({
      type:"dot",
      x, y, vx:Math.cos(ang)*speed, vy:Math.sin(ang)*speed - 1.4*big,
      size: 2.0 + Math.random()*3.8*big, alpha: 1.0,
...
function addOcto(x,y,many=false){
  ...
  for(let i=0;i<n;i++){
    sparks.push({
      type:"emoji", x: x + (Math.random()-0.5)*36, y: y+8,
      vx:(Math.random()-0.5)*1.4, vy:-1.7 - Math.random()*1.0,
      size: 5+Math.random()*2.6, alpha:1, t0:now(), life:2000+Math.random()*400
    });
  }
  ensureSparkLoop();
}
function fireworkFor(score, centsAbs, xy){
  const base = Math.round(40 * Math.exp((score-85)/5.5)); // さらに強め
  const count = clamp(base, 40, 720);
  let col, flash;
  ...
  addBurst(xy.x, xy.y, {count, life: 2000, color: col, big: (score>=98?1.8:1.3)});
...
// バージョン
ui.ver.textContent = "v2.0.0";

// 権限・許可ダイアログ再掲
function onModeBackToScale(){
  state.mode="scale"; document.body.classList.remove("arcade"); ui.game.setAttribute("aria-pressed","false");
  loadExercise(); gateBack();
}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  ['pointerup','touchend','click'].forEach(ev=>b.addEventListener(ev,h));
});
