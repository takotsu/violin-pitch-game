// app.js
// 442Hz厳密・YAMAHA系針サーボ・RMS閾値未満は無評価・二小節ページング・バックグラウンド即停止（許可要求もしない）・エラーは下部（重複なし）
...
// 可視性の全方位フック＋監視タイマで取りこぼしゼロに
const visStop=()=>{ if(document.visibilityState!=="visible") suspendAll("hidden"); };
["visibilitychange","webkitvisibilitychange"].forEach(ev=>window.addEventListener(ev,visStop));
...
setInterval(()=>{ if ((document.visibilityState!=="visible" || !document.hasFocus()) && mediaStream){ suspendAll("watchdog"); } }, 800);
