...
async function preflightChecks(){
  ...
  if(!(window.Vex && window.Vex.Flow)) notify('五線譜ライブラリの読込に失敗しました。再読み込みしてください。','error',6000);
  try{
...
function nearestNoteName(freq){
  if(!freq) return "A4";
  const n = Math.round(12*Math.log2(freq/A4_REF_HZ))+ 9 + 4*12; // A4 index base
...
function drawTunerStaff(){ renderCtx = renderSingle("A4"); } // シンプルな1全音符譜面
...
/***** スケール進行（合格閾値で前進） *****/
let inScale=false, scaleNotes=[], idx=0, firstScores=[], passThreshold=90;

/***** メトロノーム（初期OFF・音量強化） *****/
let clickBufStrong=null, clickBufWeak=null, metroTimer=null, metroOn=false;
function makeClickBuffers(){
  const sr=ac.sampleRate;
  const mk=(freq,gain)=>{ // 短いビープ＋クリップノイズの合成
    const len=Math.floor(0.05*sr);
    const buf=ac.createBuffer(1,len,sr), d=buf.getChannelData(0);
    for(let i=0;i<len;i++){
      const t=i/sr;
      const beep=Math.sin(2*Math.PI*freq*t)*0.9;
      const noise=(Math.random()*2-1)*0.6;
      let v=(beep+noise)*gain;
      v=Math.max(-0.99,Math.min(0.99,v));
      d[i]=v*Math.exp(-40*i/len);
    }
    return buf;
  };
  clickBufStrong=mk(1500,1.0);
  clickBufWeak  =mk(1200,0.9);
}
function playClick(strong=false){
  if(!ac || ac.state!=='running') return;
  const src=ac.createBufferSource(), g=ac.createGain();
  src.buffer= strong? clickBufStrong : clickBufWeak;
  g.gain.value=1.0; // 大きめ
...
function onPitchMessage(ev){
  ...
  let fTarget, cents, sc;

  if(modeSel.value==='tuner'){
    // 近傍音を自動ターゲット
    const nn = nearestNoteName(f0||A4_REF_HZ);
    fTarget = noteToFreq(nn);
    cents = (f0? centsDiff(f0, fTarget): 0);
    sc = scoreFromCents(Math.abs(cents));
    bigScoreEl.textContent=String(sc); colorScore(sc);
    feedbackEl.textContent = `ターゲット: ${nn}（A=442）` + encourage(sc);
    updateNeedle(cents);
    drawKaraoke(cents);
    return;
  }
  ...
  if(inScale){
    fTarget = noteToFreq(scaleNotes[idx]);
    cents = centsDiff(f0, fTarget);
    sc = scoreFromCents(Math.abs(cents));
    bigScoreEl.textContent=String(sc); colorScore(sc);
    feedbackEl.textContent = `音${idx+1}/${scaleNotes.length}` + encourage(sc);
    updateNeedle(cents);
    drawKaraoke(cents);
    ...
    if(sc >= passThreshold){
      idx++;
      if(idx >= scaleNotes.length){ finishScale(); return; }
    ...
    sc = scoreFromCents(Math.abs(cents));
    bigScoreEl.textContent=String(sc); colorScore(sc);
    feedbackEl.textContent = `課題音：${noteSel.value}` + encourage(sc);
    updateNeedle(cents);
    drawKaraoke(cents);
  }
}
...
async function start(){
  ...
  try{
    ...
    metroOn = metroToggle.checked; startMetronome();
    requestAnimationFrame(drawOsc);
...
modeSel.addEventListener('change', ()=>{ if(!running){ 
  if(modeSel.value==='g_scale_4bars') drawScale();
  else if(modeSel.value==='single') drawSingle();
  else drawTunerStaff();
}});
...
noteSel.addEventListener('change', ()=>{ targetFreq=noteToFreq(noteSel.value); if(!running && modeSel.value==='single'){ drawSingle(); }});
