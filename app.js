// app.js v0-3 — 1段表示の自動ページング + 0.5秒安定保持で合格 + リアルタイムチューナー + モバイル全画面
...
function detectPitch(){
  ...
  if(bestLag>0){
    ...
    return {freq: audio.sampleRate/lag, rms, db};
  }
...
function freqToNoteName(f){
  ...
  const m = Math.round(12*Math.log2(f/442) + 57); // A4(442)→MIDI69、ここは相対基準で+12*(log2) + (69-12*log2(442/442))→+57
...
// —— ポジションガイド（標準しきい値）：G3–G4=I、〜D5=III、〜G5=V、〜B5=VII、〜D6=IX、それ以上=XI ——
// MIDI番号のしきい値で分類（A4=442換算）
function posLabelForMidi(m){
  if(m <= 67) return "I";
  if(m <= 74) return "III";
  if(m <= 79) return "V";
  if(m <= 83) return "VII";
...
function midiFromFreq(f){ return Math.round(12*Math.log2(f/442) + 57); }

function buildPosLabels(notes, key){
  return notes.map(n=>{
    const f = letterFreq(n.letter, n.octave, key);
    return posLabelForMidi(midiFromFreq(f));
  });
}
...
function resetSession(key){
  ...
  const labels = buildPosLabels(sc.notes, key);

  session={
    key, notes:sc.notes, vex, posLabels:labels,
    idx:0, pass:parseInt(selPass.value,10)||90,
    results:Array(sc.notes.length).fill(null),
    page:0, perPage:24, advancedAt:0,
    lockTill:0, stableMs:0, lastCent:null
  };

  pageCtx = renderPage({key:session.key, vexKeys:session.vex, page:session.page, perPage:session.perPage, posLabels:session.posLabels});
...
function advance(){
  if(session.idx < session.notes.length-1){
    session.idx += 1;
    session.stableMs = 0; session.lastCent = null;
    turnPageIfNeeded();
...
// 全画面（iOS Safari でも要素フルスクリーン可）
btnFull.addEventListener("click", async ()=>{
  const el = document.documentElement;
  try{
    if(!document.fullscreenElement){ await el.requestFullscreen?.(); btnFull.textContent="終了"; }
    else { await document.exitFullscreen?.(); btnFull.textContent="全画面"; }
  }catch(e){ pushErr("fullscreen: "+e.message); }
});
...
window.addEventListener("resize",()=>{ const wrap=document.getElementById("staff-wrap"); fxCanvas.width=wrap.clientWidth; fxCanvas.height=wrap.clientHeight; if(session){ turnPageIfNeeded(); recolorPage(pageCtx, session.idx % session.perPage, session.results[session.idx]); }});
