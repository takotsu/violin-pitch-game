// app.js v0-3c — VexFlowが無くても描画。マイク接続状態を明示。0.5秒安定保持で合格。
...
const errors=[]; const pushErr=(e)=>{ const t=new Date().toISOString().replace("T"," ").slice(0,19); errors.push(`${t} : ${e}`); console.warn(e); };
...
function detectPitch(){
  ...
  if(!mediaStream){ advice.textContent="マイク未接続"; advice.style.color="#ff9f0a"; }
...
function freqToNoteName(f){ if(!f||!isFinite(f)) return {name:"—",oct:"-"}; const m=Math.round(12*Math.log2(f/442)+57); const names=["C","C#","D","D#","E","F","F#","G","G#","A","A#","B"]; return {name:names[(m+1200)%12],oct:Math.floor(m/12)-1}; }
function sparks(){ const cv=fxCanvas, ctx=cv.getContext("2d"); if(!cv.width||!cv.height){ const wrap=document.getElementById("staff-wrap"); cv.width=wrap.clientWidth; cv.height=wrap.clientHeight; } const N=70,P=[]; for(let i=0;i<N;i++){ const a=Math.random()*Math.PI*2, sp=2+Math.random()*2.2; P.push({x:cv.width*0.5,y:cv.height*0.4, vx:Math.cos(a)*sp, vy:Math.sin(a)*sp, life:0}); } let t0=performance.now(); function step(now){ const dt=Math.min(32,now-t0)/1000; t0=now; ctx.clearRect(0,0,cv.width,cv.height); P.forEach(p=>{ p.life+=dt; p.vy+=2.6*dt; p.x+=p.vx; p.y+=p.vy; const a=Math.max(0,1-p.life/0.6); ctx.fillStyle=`rgba(114,241,165,${a})`; ctx.beginPath(); ctx.arc(p.x,p.y,2,0,Math.PI*2); ctx.fill(); }); if(P[0].life<0.6) requestAnimationFrame(step); else ctx.clearRect(0,0,cv.width,cv.height);} requestAnimationFrame(step); if(navigator.vibrate) try{ navigator.vibrate(10);}catch{} }

function setProg(idx,len){ prog.textContent=`音 ${idx+1}/${len}${idx<24?"（上行）":"（下行）"}`; }
...
function resetSession(key){
  ...
  session={
    ...
    page:0, perPage:24, stableMs:0
  };

  pageCtx = renderPage({key:session.key, vexKeys:session.vex, objs:session.notes, page:session.page, perPage:session.perPage});
  setProg(session.idx, session.notes.length);
  advice.textContent="待機中…"; advice.style.color="#ffccd5"; bigScore.textContent="0";
  // fallback用：全音列も持たせておく
  if(pageCtx && pageCtx.mode==="fallback") pageCtx.__allObjs = session.notes;
}

function turnPageIfNeeded(){
  const newPage = Math.floor(session.idx/session.perPage);
  if(!pageCtx || newPage===session.page) return;
...
function advance(){
  if(session.idx < session.notes.length-1){
    session.idx += 1; session.stableMs = 0;
    turnPageIfNeeded();
    recolorPage(pageCtx, session.idx % session.perPage, session.results[session.idx]);
    setProg(session.idx, session.notes.length);
  }
}
...
function loop(){
  ...
  if(freq>0 && session){
    const near=freqToNoteName(freq); tunNote.textContent=near.name; tunOct.textContent=near.oct;
    ...
    if(abs<=15){
      ...
      recolorPage(pageCtx, session.idx % session.perPage, session.results[session.idx]);
      if(session.stableMs>=500 && (session.results[session.idx] >= session.pass)){ sparks(); advance(); }
    }else{ session.stableMs=0; }
  }else if(!mediaStream){
    advice.textContent="マイク未接続"; advice.style.color="#ff9f0a";
  }else{
...
permit.addEventListener("click", async ()=>{
  ...
  if(!ok) gate.setAttribute("aria-hidden","false");
});
...
window.addEventListener("visibilitychange",()=>{
  ...
  else if(!mediaStream){ gate.setAttribute("aria-hidden","false"); }
});
