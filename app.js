// app.js v0-3a — VexFlowロード待ち + 安定保持判定 + モバイル最適化（全画面ボタンは削除）
...
function buildPosLabels(notes, key, letterFreqFn){
  return notes.map(n=>{
    const f = letterFreqFn(n.letter, n.octave, key);
...
async function resetSession(key){
  // VexFlow が使えるまで待つ
  pageReady = await waitVexFlow();
  const sc = makeMajorScale3Oct(key);
  const vex = toVexKeys(sc.notes, key);
  const labels = buildPosLabels(sc.notes, key, letterFreq);
...
btnStart.onclick=async ()=>{
  try{
    ...
    if(!pageReady) await waitVexFlow();
    if(pageReady && !pageCtx) pageCtx = renderPage({key:session.key, vexKeys:session.vex, page:session.page, perPage:session.perPage, posLabels:session.posLabels});
