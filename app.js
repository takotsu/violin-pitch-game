...
window.addEventListener("unhandledrejection",ev=>{ pushErr(ev.reason?.message||String(ev.reason)); showErrorModal(); });
...
async function requestWakeLock(){
  try{ if('wakeLock' in navigator){ wakeLock = await navigator.wakeLock.request('screen'); wakeLock.addEventListener('release',()=>{ /* noop */}); } }
  catch{ /* iOS Safari 未対応でも無音動画が補助 */ }
}
function releaseWakeLock(){ try{ wakeLock?.release(); }catch{} }
...
async function openMic(){
  ...
  // スリープ抑止（ユーザー操作後ならiOSでも再生可）
  try{ await noSleep.play(); }catch{}
  await requestWakeLock();
}
function closeMic(){
  try{ noSleep.pause(); noSleep.currentTime=0; }catch{}
  releaseWakeLock();
...
function detectPitch(){
  ...
  // dB表示：dBFS(<=0) を 100 加算して 0..100 に持ち上げ、校正値を加算
...
window.addEventListener("visibilitychange",()=>{
  if(document.hidden){
    running=false; if(rafId) cancelAnimationFrame(rafId); btnStart.disabled=false; btnStop.disabled=true;
