...
function ensureCanvasSize(){
  const w=staffDiv.clientWidth||1024;
  karaokeCanvas.width=w; karaokeCanvas.height=160;
}
...
function renderKeySignature(svg, key, left, staffTop, space){
  ...
  if(sig.sharps.length){
    for(let i=0;i<sig.sharps.length;i++){ const p=SHARP_POS[i]; drawSharp(p.L,p.o); }
  }else if(sig.flats.length){
    for(let i=0;i<sig.flats.length;i++){ const p=FLAT_POS[i]; drawFlat(p.L,p.o); }
  }
  return x-left;
}
...
function renderFallbackScale(key, noteObjs, highlightIdx=-1, nextIdx=null){
  staffDiv.innerHTML="";
  const w=staffDiv.clientWidth||1024, h=160; const svg=mkSvg(w,h); staffDiv.appendChild(svg);
  const left=18, right=w-14, staffTop=22, space=12, staffBottom=staffTop+space*4;
...
export function renderScale(keySignature, vexKeys, noteObjs=null){
  ...
  if(VF){
    ...
    const w=staffDiv.clientWidth||1024, h=160; renderer.resize(w,h);
    const ctx = renderer.getContext();
    const stave = new VF.Stave(10,10,w-20);
    // ここで clef を追加しない＝ト音記号を描かない
