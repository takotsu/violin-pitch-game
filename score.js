...
function renderFallback({ key, notes16 }){
  ...
  const top=70, space=16, left=18, right=w-18, bottom=top+space*4;
  ...
  notes16.forEach((n,i)=>{
    ...
    // 加線
    const pos = (bottom - y)/(space/2);
    if(pos<-2){ for(let k=-2;k>=pos; k-=2){ line(svg, x-9, bottom + (Math.abs(k)/2-1)*space, x+9, bottom + (Math.abs(k)/2-1)*space, "#e8eef7",1.2); } }
  ...
  return {
    ...
    badge(i, symbol){
      ...
      if(cache.mode==="vex"){
        ...
        const x = bb.getX() + bb.getW() + 6; const y = bb.getY() + 6;
    ...
    getXY(i){
      if(cache.mode==="vex"){
        ...
        return cache.anchors[i] || {x:0,y:0};
      }
    }
  };
}
