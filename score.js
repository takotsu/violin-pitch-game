...
export function highlightIndex(renderCtx, idx){
  ...
  if(VF && (VFCache||renderCtx)?.notes?.length){
    ...
    const voice1=new window.Vex.Flow.Voice({num_beats:24,beat_value:4}).setMode(window.Vex.Flow.Voice.Mode.SOFT).addTickables(notes.slice(0,24));
    const voice2=new window.Vex.Flow.Voice({num_beats:24,beat_value:4}).setMode(window.Vex.Flow.Voice.Mode.SOFT).addTickables(notes.slice(24,48));
    new window.Vex.Flow.Formatter().joinVoices([voice1]).format([voice1], w-36);
    new window.Vex.Flow.Formatter().joinVoices([voice2]).format([voice2], w-36);
    voice1.draw(ctx,stave1); voice2.draw(ctx,stave2);
