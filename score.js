...
export function renderScale(key,vexKeys,objs){
  ...
  if(VF){
    ...
    const s1=new VF.Stave(6,6,w-12);           // clef/time は出さない
    s1.addKeySignature(key).setContext(ctx).draw();
    const s2=new VF.Stave(6,166,w-12);
    ...
    if(bbox.height+18>baseH){
      const scale=(baseH-18)/bbox.height;
  ...
  systems.forEach((sys,si)=>{
    ...
    sys.notes.forEach((n,i)=>{
      ...
      const yi=(()=>{
        ...
        const bottomLine=top+space*4;
        const steps=(n.octave-4)*7 + (idx(n.letter)-idx("E"));
        return bottomLine - (steps*space/2);
      })();
      if(yi<top||yi>bottom) drawLedger(allGroup,xi,yi,top,bottom,space);
      const h=head(allGroup,xi,yi,(si===0&&i===0)?"#22c55e":"#e8eef7");
      const st=stem(allGroup,xi,yi,10,(si===0&&i===0)?"#22c55e":"#e8eef7");
  ...
  const bb=allGroup.getBBox(); const fit=Math.min(1,(baseH-16)/bb.height);
