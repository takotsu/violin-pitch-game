...
function notehead(svg,x,y,fill="#e8eef7",rX=5.6,rY=4.0,rot=-20){ const ns="http://www.w3.org/2000/svg"; const e=document.createElementNS(ns,"ellipse"); e.setAttribute("cx",x); e.setAttribute("cy",y); e.setAttribute("rx",rX); e.setAttribute("ry",rY); e.setAttribute("fill",fill); e.setAttribute("transform",`rotate(${rot},${x},${y})`); svg.appendChild(e); return e; }
function stem(svg,x,yUp,len=18,stroke="#e8eef7"){ return line(svg,x+7,yUp, x+7, yUp-len, stroke, 1.6); }

let fbState=null; let VFCache=null;
...
function renderKeySignature(svg, key, left, staffTop, space){
  ...
  let x=left;
  const drawSharp=(L,o)=>{ text(svg,x, yFor(L,o)+4, "♯", 17, "800","left","#e8eef7"); x+=9; };
  const drawFlat =(L,o)=>{ text(svg,x, yFor(L,o)+4, "♭", 17, "800","left","#e8eef7"); x+=9; };
  if(sig.sharps.length){ for(let i=0;i<sig.sharps.length;i++){ const p=SHARP_POS[i]; drawSharp(p.L,p.o); } }
  if(sig.flats.length){ for(let i=0;i<sig.flats.length;i++){ const p=FLAT_POS[i]; drawFlat(p.L,p.o); } }
  return x-left;
}

function drawLedgerLines(svg, xi, yi, top, bottom, space, color="#a7c7dd"){
  const short=18, w=1.2;
...
function renderFallbackScaleTwoSystems(key, noteObjs, highlightIdx=0){
  const staffDiv=document.getElementById('staff');
  staffDiv.innerHTML="";
  const w=staffDiv.clientWidth||820, h=560; const svg=mkSvg(w,h); staffDiv.appendChild(svg);
  const systems=[{top:22,left:8,right:w-8,notes:noteObjs.slice(0,24)},
                 {top:286,left:8,right:w-8,notes:noteObjs.slice(24,48)}];
  const nodes=[]; const space=12;

  systems.forEach((sys,sIdx)=>{
    const {top,left,right,notes}=sys; const bottom=top+space*4;
    for(let i=0;i<5;i++) line(svg,left, top+space*i, right, top+space*i, "#e8eef7", 1.1);
    const ksW=renderKeySignature(svg, key, left+14, top, space);
    const innerLeft=left+14+ksW+8, innerRight=right-6;
    ...
    notes.forEach((n,i)=>{
      ...
      const gi=sIdx*24+i; const color=(gi===highlightIdx)?"#22c55e":"#e8eef7";
      if(yi<top||yi>bottom) drawLedgerLines(svg,xi,yi,top,bottom,space,color);
      const head=notehead(svg,xi,yi,color);
      const stm = stem(svg,xi,yi-3,18,color);    /* ← 生成した stem を… */
      nodes[gi] = { head, stem: stm };           /* ← 正しく保存（バグ修正） */
      if((i+1)%8===0 && i<notes.length-1){ const bx=innerLeft+stepX*(i+1); line(svg,bx,top,bx,bottom,"#7aa2c1",1.1); }
    });
  });
...
export function renderScale(keySignature, vexKeys, noteObjs=null){
  ...
  if(VF){
    ...
    const w=staffDiv.clientWidth||820, h=560; renderer.resize(w,h);
    const ctx=renderer.getContext();
    const stave1=new VF.Stave(6,10,w-12); stave1.addKeySignature(keySignature); stave1.setContext(ctx).draw();
    const stave2=new VF.Stave(6,282,w-12); stave2.addKeySignature(keySignature); stave2.setContext(ctx).draw();
    ...
    const voice1=new VF.Voice({num_beats:24,beat_value:4}).setMode(VF.Voice.Mode.SOFT).addTickables(notes1);
    const voice2=new VF.Voice({num_beats:24,beat_value:4}).setMode(VF.Voice.Mode.SOFT).addTickables(notes2);
    new VF.Formatter().joinVoices([voice1]).format([voice1], w-36);
    new VF.Formatter().joinVoices([voice2]).format([voice2], w-36);
    voice1.draw(ctx,stave1); voice2.draw(ctx,stave2);

    VFCache={renderer, ctx, stave1, stave2, notes:[...notes1,...notes2]};
    highlightIndex(VFCache,0); fbState=null; return VFCache;
  }else{
    const objs = noteObjs || (vexKeys||[]).map(vk=>{ const m=vk.match(/^([A-G])\/(\d)$/); return {letter:m?m[1]:"A", octave:m?+m[2]:4}; });
    const rc = renderFallbackScaleTwoSystems(keySignature, objs, 0);
...
export function highlightIndex(renderCtx, idx){
  ...
  if(VF && (VFCache||renderCtx)?.notes?.length){
    const cache=VFCache||renderCtx; const {renderer,stave1,stave2}=cache;
    const w=document.getElementById('staff').clientWidth||820; renderer.resize(w,560);
    ...
    const v1=new window.Vex.Flow.Voice({num_beats:24,beat_value:4}).setMode(window.Vex.Flow.Voice.Mode.SOFT).addTickables(notes.slice(0,24));
    const v2=new window.Vex.Flow.Voice({num_beats:24,beat_value:4}).setMode(window.Vex.Flow.Voice.Mode.SOFT).addTickables(notes.slice(24,48));
    new window.Vex.Flow.Formatter().joinVoices([v1]).format([v1], w-36);
    new window.Vex.Flow.Formatter().joinVoices([v2]).format([v2], w-36);
    v1.draw(ctx,stave1); v2.draw(ctx,stave2);
    VFCache={renderer,ctx,stave1,stave2,notes}; return;
  }
  if(fbState?.nodes){
    fbState.nodes.forEach((n,i)=>{
      const c=(i===idx)?"#22c55e":"#e8eef7";
      if(!n) return;
      if(n.head) n.head.setAttribute("fill",c);
      if(n.stem && n.stem.setAttribute) n.stem.setAttribute("stroke",c); // 念のため安全化
    });
  }
}
