...
function renderFallbackScaleTwoSystems(key, noteObjs, highlightIdx=0){
  staffDiv.innerHTML="";
  const w=staffDiv.clientWidth||660, h=300; const svg=mkSvg(w,h); staffDiv.appendChild(svg);

  const systems = [
    {top:30, left:12, right:w-12, notes: noteObjs.slice(0,16)},
    {top:170, left:12, right:w-12, notes: noteObjs.slice(16,32)}
  ];
  const nodes=[];
...
export function renderScale(keySignature, vexKeys, noteObjs=null){
  ...
  if(VF){
    ...
    const w=staffDiv.clientWidth||660, h=300; renderer.resize(w,h);
    ...
    const stave2 = new VF.Stave(10,160,w-20);
