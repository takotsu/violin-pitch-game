...
function renderFallbackScaleTwoSystems(key, noteObjs, highlightIdx=0){
  ...
  systems.forEach((sys,sIdx)=>{
    ...
    notes.forEach((n,i)=>{
      ...
      const head=notehead(svg,xi,yi,color); const stm=stem(svg,xi,yi-3,18,color);
      nodes[gi]={head,stem};
...
export function highlightIndex(renderCtx, idx){
  ...
  if(VF && (VFCache||renderCtx)?.notes?.length){
    ...
    const w=staffDiv.clientWidth||820; renderer.resize(w,560);
  ...
  if(fbState?.nodes){ fbState.nodes.forEach((n,i)=>{ const c=(i===idx)?"#22c55e":"#e8eef7"; if(!n) return; n.head.setAttribute("fill",c); n.stem.setAttribute("stroke",c); }); }
}
