...
function renderFallbackScaleTwoSystems(key, noteObjs, highlightIdx=0){
  const staffDiv = document.getElementById('staff');
  staffDiv.innerHTML="";
  const w=staffDiv.clientWidth||820, h=520; const svg=mkSvg(w,h); staffDiv.appendChild(svg);

  const systems=[{top:26,left:10,right:w-10,notes:noteObjs.slice(0,24)},
                 {top:270,left:10,right:w-10,notes:noteObjs.slice(24,48)}];
  const nodes=[];
  const space=12; // ★ 間隔を狭く
  ...
  systems.forEach((sys,sIdx)=>{
    ...
    const ksW = renderKeySignature(svg, key, left+18, top, space);
    const innerLeft = left+18+ksW+8, innerRight = right-8;
...
export function renderScale(keySignature, vexKeys, noteObjs=null){
  ...
  if(VF){
    ...
    const w=staffDiv.clientWidth||820, h=520; renderer.resize(w,h);
    const ctx=renderer.getContext();
    const stave1=new VF.Stave(8,18,w-16); stave1.addKeySignature(keySignature); stave1.setContext(ctx).draw();
    const stave2=new VF.Stave(8,268,w-16); stave2.addKeySignature(keySignature); stave2.setContext(ctx).draw();
