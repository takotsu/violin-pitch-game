...
function renderKeySignature(svg, key, left, top, space){
  const keyMap = {
    "C":{s:0}, "G":{s:1}, "D":{s:2}, "A":{s:3}, "E":{s:4}, "B":{s:5}, "F#":{s:6},
...
function renderFallback({ key, notes16 }){
  staffDiv.innerHTML="";
  const w=staffDiv.clientWidth||780, h=staffDiv.clientHeight||380;

  let top=70, space=16, left=18, right=w-18;
  const yTrial=(letter,oct,top0)=>{ const idx=(l)=>["C","D","E","F","G","A","B"].indexOf(l); const s=(oct-4)*7+(idx(letter)-idx("E")); return (top0+space*4)-(s*space/2); };
  let minY=1e9; for(const n of notes16){ const y=yTrial(n.letter,n.octave,top); if(y<minY) minY=y; } // 高音見切れ補正
...
function renderVex({ key, vexKeys16 }){
  ...
  const w=staffDiv.clientWidth||780, h=staffDiv.clientHeight||380; renderer.resize(w,h);
