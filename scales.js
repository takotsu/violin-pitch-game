...
export function makeMajorScale3Oct(key){
  ...
  // 現在の最小・最大
  const bounds=(s)=>{ let mn=Infinity,mx=0; for(const n of s){ const f=letterFreq(n.letter,n.octave,key); if(f<mn)mn=f; if(f>mx)mx=f; } return [mn,mx]; };
  let [minF,maxF]=bounds(seq);

  // 2^k の一括シフト
  const kLow  = Math.ceil( Math.log2(G3/minF) );
  const kHigh = Math.floor( Math.log2(E7/maxF) );
  let k = Math.min(Math.max(kLow, -8), kHigh);
  if(!Number.isFinite(k)) k=0;
  if(k!==0) seq = seq.map(n=>({...n, octave:n.octave+k}));

  // 最終安全ループ（必ず G3..E7 に入るまで±1オクターブ調整）
  let guard=0; [minF,maxF]=bounds(seq);
  while(minF < G3 && guard++<16){ seq = seq.map(n=>({...n, octave:n.octave+1})); [minF,maxF]=bounds(seq); }
  guard=0;
  while(maxF > E7 && guard++<16){ seq = seq.map(n=>({...n, octave:n.octave-1})); [minF,maxF]=bounds(seq); }
