// scales.js
// violin_scales_3oct_levels.js（ユーザー提供データをそのまま採用）
...
export function parseNote(s){ const m = s.match(/^([A-G])([#b]?)(\d)$/); if(!m) throw new Error("Parse error: "+s); return {letter:m[1], acc:m[2]||"", octave:+m[3]}; }
export function letterFreqWithAcc({letter, acc, octave}, a4=A4){ let semi=NAT[letter]+(acc==="#"?1:(acc==="b"?-1:0)); const n=(octave-4)*12+(semi-9); return a4*Math.pow(2,n/12); }
...
// 利用可能キー一覧
export function getKeys(scaleType, level){
  const ds = dataset(scaleType, level);
